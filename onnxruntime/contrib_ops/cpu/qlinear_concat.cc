...
QLinearConcat::QLinearConcat(const OpKernelInfo& info) : OpKernel(info), ConcatBase(info) {
  ...
  if (input_def_count < 8 || (input_def_count - 8) % 3 != 0) {
    ORT_ENFORCE(false, "At lease two inputs are needed, and each input must be (tensor, scale, zero_point) tuple!");
  }
  ...
  for (size_t def_index = 2; def_index < input_def_count; def_index += 3) {
    ...
    bool get_x_scale = info.TryGetConstantInput(def_index + 1, &tensor_x_scale);
    bool get_x_zero_point = !info.TryGetConstantInput(def_index + 2, &tensor_x_zero_point);
    if (!get_x_scale || !get_x_zero_point) {
      return;
    }
    if ((is_signed_int8 && !tensor_x_zero_point->IsDataType<int8_t>()) ||
        (!is_signed_int8 && !tensor_x_zero_point->IsDataType<uint8_t>())) {
      ORT_ENFORCE(false, "Wrong input type encountered for zero point input def @", def_index + 2);
    }
    if (!tensor_y_scale->IsDataType<float>()) {
      ORT_ENFORCE(false, "Input scale is not float for input def @", def_index + 1)
    }
...
Status QLinearConcat::Compute(OpKernelContext* ctx) const {
  ...
  if (input_count_x3 < 6 || (input_count_x3 % 3) != 0) {
    ORT_ENFORCE(false, "At lease two inputs are needed, and each input must be (tensor, scale, zero_point) tuple!");
  }

  // Hold pointers to the input tensors to be used in the PrepareForCompute() step
  auto input_count = input_count_x3 / 3;
  std::vector<std::vector<uint8_t>> dynamic_lookup_tables_(input_count);

  std::vector<const Tensor*> input_tensors(input_count);
  for (size_t input_index = 0; input_index < input_count; ++input_index) {
    auto tuple_start = input_index * 3;
    input_tensors[input_index] = ctx->Input<Tensor>(tuple_start);

    // Build lookup table for non-const parameters (scale + zero point)
    if (fixed_lookup_tables_.size() == 0) {
      // Check tensor type first
      const Tensor* tensor_x_scale = ctx->Input<Tensor>(tuple_start + 1);
      if (!tensor_x_scale->IsDataType<float>()) {
        ORT_ENFORCE(false, "Input scale is not float for input def @", input_index + 1)
      }
      const Tensor* tensor_x_zero_point = ctx->Input<Tensor>(tuple_start + 2);
      if ((is_signed_int8 && !tensor_x_zero_point->IsDataType<int8_t>()) ||
          (!is_signed_int8 && !tensor_x_zero_point->IsDataType<uint8_t>())) {
        ORT_ENFORCE(false, "Wrong input type encountered for zero point input def @", input_index + 2);
      }

      dynamic_lookup_tables_[input_index].resize(256);
      if (is_signed_int8) {
        QlinearBuildLookupTable<int8_t>(
            dynamic_lookup_tables_[input_index].data(), tensor_x_scale, tensor_x_zero_point,
            tensor_y_scale, tensor_y_zero_point, identity_float);
      } else {
        QlinearBuildLookupTable<uint8_t>(
            dynamic_lookup_tables_[input_index].data(), tensor_x_scale, tensor_x_zero_point,
  ...
  for (int input_index = 0; input_index < input_count; input_index++) {
    ...
    for (size_t idx_copy = 0, end = input_size / input_axis_pitch; idx_copy < end; ++idx_copy) {
      const uint8_t* table = (fixed_lookup_tables_[input_index].size() > 0)
                                 ...
                                 : dynamic_lookup_tables_[input_index].data();

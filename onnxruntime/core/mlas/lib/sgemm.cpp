    ...
    MLAS_THREADPOOL* ThreadPool)
{
    ...
    MlasTrySimpleParallel(ThreadPool, ThreadsPerGemm * static_cast<ptrdiff_t>(BatchSize), 
        [=](ptrdiff_t tid) {
        ptrdiff_t GemmIdx = tid / ThreadsPerGemm;
        ptrdiff_t ThreadIdx = tid % ThreadsPerGemm;
        MlasSgemmThreaded(BIsPacked, ThreadCountM, ThreadCountN, TransA, TransB, M, N, K, &(Data[GemmIdx]), ThreadIdx);
    });
}
